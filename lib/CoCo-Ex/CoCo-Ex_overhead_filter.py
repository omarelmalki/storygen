        ...
        with open(outpath, "a", encoding="utf-8") as f:
            for sent, rest in grouped:
                ...
                for nr in node_line_numbers:
                    # Only keep noun phrases
                    if phrases_mapping[rest.loc[nr, "PHRASE-TYPE"]] == 'NP':
                        # Only add lemmatized version if it exists
                        if rest.loc[nr, "NODE-LEMMATIZED"] != "None":
                            nodes.add(rest.loc[nr, "NODE-LEMMATIZED"])
                        else:
                            nodes.add(rest.loc[nr, "NODE"])

                f.write("{}\t{}\t{}\t{}\n".format(sent[0].split("_")[0], sent[0].split("_")[1], sent[1], "".join(
