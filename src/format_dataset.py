...
source_df = roc_stories_df[['sentence1', 'sentence2']]
target_df = roc_stories_df[['sentence3', 'sentence4', 'sentence5']]
...
for split in splits:
    split[1].reset_index(drop=True).to_csv(f'../data/storygen/{split[0]}/source.csv', sep=',', header=False)
    split[2].reset_index(drop=True).to_csv(f'../data/storygen/{split[0]}/target.csv', sep=',', header=False)
